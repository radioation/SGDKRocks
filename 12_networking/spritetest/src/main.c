...
int main()
{
	...
	{ 
		...
		if( joypadState & BUTTON_LEFT ) {
			input += 0b00000001 ;
		}else if( joypadState & BUTTON_RIGHT ) {
			input += 0b00000010 ;
		}else if( joypadState & BUTTON_UP ) {
			input += 0b00000100 ;
		}else if( joypadState & BUTTON_DOWN ) {
			input += 0b00001000 ;
		}
		sprintf(str, "i: %d", input); // Convert
		VDP_drawText(str, cursor_x+5, cursor_y+2);
		sprintf(str, "t: %d", tick); // Convert
		VDP_drawText(str, cursor_x +5, cursor_y+4);
