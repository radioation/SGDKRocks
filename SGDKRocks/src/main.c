...
#define PI_1 FIX32(3.14159)
#define PI_2 FIX32(1.57079)

/*
fix32 FastArcTan(fix32 x)
{
    // return M_PI_4*x - x*(fabs(x) - 1)*(0.2447 + 0.0663*fabs(x));
    return fix32Sub( M_PI_4*x ,  fix32Mul( x, fix32Mul(  fix32Sub( abs(x),1)  , fix32Add(FIX32(0.2447) , fix32Mul(FIX32(0.0663),abs(x)) )) ) );
...
void update()
{
		...
		for (u16 i = 0; i < MAX_ROCKS; ++i)
		{
				if (rocks[i].active == TRUE)
				{
						...
						if (rocks[i].pos_x < FIX32(-32))
						{
							rocks[i].pos_x = FIX32(MAP_WIDTH);
						}
						else if (rocks[i].pos_x > FIX32(MAP_WIDTH))
						{
							rocks[i].pos_x = FIX32(-32);
						}

						rocks[i].pos_y += rocks[i].vel_y;
						if (rocks[i].pos_y < FIX32(-32))
						{
							rocks[i].pos_y = FIX32(MAP_HEIGHT);
						}
						else if (rocks[i].pos_y > FIX32(MAP_HEIGHT))
						{
							rocks[i].pos_y = FIX32(-32);
						}
		...
		for (u16 i = 0; i < MAX_ROCKS; ++i)
		{
				if (midRocks[i].active == TRUE)
				{
						...
						if (midRocks[i].pos_x < FIX32(-32))
						{
							midRocks[i].pos_x = FIX32(MAP_WIDTH);
						}
						else if (midRocks[i].pos_x > FIX32(MAP_WIDTH))
						{
							midRocks[i].pos_x = FIX32(-32);
						}

						midRocks[i].pos_y += midRocks[i].vel_y;
						if (midRocks[i].pos_y < FIX32(-32))
						{
							midRocks[i].pos_y = FIX32(MAP_HEIGHT);
						}
						else if (midRocks[i].pos_y > FIX32(MAP_HEIGHT))
						{
							midRocks[i].pos_y = FIX32(-32);
						}
		...
		for (u16 i = 0; i < MAX_SMALL_ROCKS; ++i)
		{
				if (smallRocks[i].active == TRUE)
				{
						...
						if (smallRocks[i].pos_x < FIX32(-32))
						{
							smallRocks[i].pos_x = FIX32(MAP_WIDTH);
						}
						else if (smallRocks[i].pos_x > FIX32(MAP_WIDTH))
						{
							smallRocks[i].pos_x = FIX32(-32);
						}

						smallRocks[i].pos_y += smallRocks[i].vel_y;
						if (smallRocks[i].pos_y < FIX32(-32))
						{
							smallRocks[i].pos_y = FIX32(MAP_HEIGHT);
						}
						else if (smallRocks[i].pos_y > FIX32(MAP_HEIGHT))
						{
							smallRocks[i].pos_y = FIX32(-32);
						}
		...
		for (u16 i = 0; i < MAX_SMALL_ROCKS; ++i)
		{
				if (xsRocks[i].active == TRUE)
				{
						...
						if (xsRocks[i].pos_x < FIX32(-32))
						{
							xsRocks[i].pos_x = FIX32(MAP_WIDTH);
						}
						else if (xsRocks[i].pos_x > FIX32(MAP_WIDTH))
						{
							xsRocks[i].pos_x = FIX32(-32);
						}

						xsRocks[i].pos_y += xsRocks[i].vel_y;
						if (xsRocks[i].pos_y < FIX32(-32))
						{
							xsRocks[i].pos_y = FIX32(MAP_HEIGHT);
						}
						else if (xsRocks[i].pos_y > FIX32(MAP_HEIGHT))
						{
							xsRocks[i].pos_y = FIX32(-32);
						}
		...
		for (u16 i = 0; i < MAX_ENEMIES; ++i)
		{
				if (enemies[i].active == TRUE)
				{
						...
						if (enemies[i].pos_x < FIX32(-32))
						{
							enemies[i].pos_x = FIX32(MAP_WIDTH);
						}
						else if (enemies[i].pos_x > FIX32(MAP_WIDTH))
						{
							enemies[i].pos_x = FIX32(-32);
						}

						enemies[i].pos_y += enemies[i].vel_y;
						if (enemies[i].pos_y < FIX32(-32))
						{
							enemies[i].pos_y = FIX32(MAP_HEIGHT);
						}
						else if (enemies[i].pos_y > FIX32(MAP_HEIGHT))
						{
							enemies[i].pos_y = FIX32(-32);
						}
...
int main(bool hard)
{
		...
		// fix32 x = FIX32(2);
		// fix32 y = FIX32(1);
		// fix32 z = fix32Div(y, x);
		// fix32 temp = ApproxAtan(z);
		...
		SPR_initEx(900);
		player.pos_x = FIX32(MAP_WIDTH / 2 - 8);
		player.pos_y = FIX32(MAP_HEIGHT / 2 - 8);
		...
		player.hitbox_x1 = FIX32(3);
		player.hitbox_y1 = FIX32(3);
		player.hitbox_x2 = FIX32(13);
		player.hitbox_y2 = FIX32(13);
		playerRotation = MIN_ROTATION_INDEX;
		// playerRotation = FIX32(12);
