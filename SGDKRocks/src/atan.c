...
fix32 ApproxAtan(fix32 z)
{
    ...
    return F32_mul( n1  +F32_mul( n2 ,F32_mul( z, z)), z);
}
            ...
            if (x > FIX32(0.0))
						...
						else if (y >= FIX32(0.0))
						{
							...
							//return fix32Add(ApproxAtan(z), PI_1);
								fix32 i =  F32_mul(FIX32(154.878),tempAtan + PI_1)+ FIX32(24.683);
								return F32_toInt(i);
						}
						else
						{
							...
							fix32 i = F32_mul(FIX32(154.878),(tempAtan-PI_1))+ FIX32(998.317);
							return F32_toInt(i);
				...
				else // Use property atan(y/x) = PI/2 - atan(x/y) if |y/x| > 1.
				{
					  ...
					  const fix32 tempAtan = ApproxAtan(z);
						if (y > FIX32(0.0))
						{
							...
							fix32 i = F32_mul(FIX32(170.0),(PI_2-tempAtan))- FIX32(11.4);
							return F32_toInt(i);
						}
						else
						{
							...
							fix32 i = ( F32_mul(FIX32(-170.0),(PI_2+tempAtan))+ FIX32(1034.4));
							return F32_toInt(i);
						}
				}
		}
